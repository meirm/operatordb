...
function print_node($root,$ident,$tab,$index)
{
	$has_sub = 0;
	for($i=0;$i<10;$i++)
	{
	...
	if($has_sub==1)
	{
		...
		for($i=0;$i<10;$i++)
		{
			if(isset($root[$i]))
			{
				...
				echo $ident.$tab."{\n";
				print_node($root[$i],$ident.$tab.$tab,$tab,$index+1);
	...
	else
	{
		...
		{
			echo $ident."*country = \"".$root['country']."\";\n";
		}
		if(isset($root['organisation']))
		{
			echo $ident."*organisation = \"".$root['organisation']."\";\n";
		}
		if(isset($root['network']))
		{
			echo $ident."*network = \"".$root['network']."\";\n";
		}
		if(isset($root['abbreviated_name']))
		{
			echo $ident."*abbreviated_name = \"".$root['abbreviated_name']."\";\n";
		}
		if(isset($root['mcc']))
		{
			echo $ident."*mcc = \"".$root['mcc']."\";\n";
		}
		if(isset($root['mnc']))
		{
			echo $ident."*mnc = \"".$root['mnc']."\";\n";
		}
		if(isset($root['sim']))
		{
			echo $ident."*sim = \"".$root['sim']."\";\n";
		}
		if(isset($root['last_update']))
		{
			echo $ident."*last_update = \"".$root['last_update']."\";\n";
		}
		if(isset($root['operator_code']))
		{
			echo $ident."*operator_code = \"".$root['operator_code']."\";\n";
		}
	}
}
